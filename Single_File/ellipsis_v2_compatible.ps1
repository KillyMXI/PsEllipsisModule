...
# PowerShell v2.0 compatible version
...
function Format-Ellipsis
{
    ...
    param(
        ...
        [parameter(Mandatory=$true)][ValidateScript({$_ -ge 3})][Alias('w')][int]$Width,
        [ValidateSet('count','length','len',ignorecase=$true)][Alias('by')][string]$Weighting,
        [ValidateRange(0.0,1.0)][Alias('p')][double]$Proportion,
        [ValidateSet('none','auto','left','right',ignorecase=$true)][Alias('cut')][string]$CutSegments,
        [ValidateScript({$_.Length -ge 1})][Alias('s')][string]$Separator,
        [ValidateScript({$_.Length -ge 1})][Alias('e')][string]$EllipsisString,
        [switch]$Strict
    )
    
    if(-not $PSBoundParameters.ContainsKey('Weighting')) { $Weighting = 'length'; }
    if(-not $PSBoundParameters.ContainsKey('Proportion')) { $Proportion = 0.5; }
    if(-not $PSBoundParameters.ContainsKey('CutSegments')) { $CutSegments = 'left'; }
    if(-not $PSBoundParameters.ContainsKey('Separator')) { $Separator = [IO.Path]::DirectorySeparatorChar; }
    if(-not $PSBoundParameters.ContainsKey('EllipsisString')) { $EllipsisString = '...'; }
    ...
    while($true)
    {
        ...
        if($deadEnd)
        {
            Write-Debug ("`n" + "-"*$Width); # ruler to check output against
